...
class Lootbox(BaseCog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self.role_ids = self.config.get("role_ids")
        # etc, can eventually move them to a dashboard/config file or something easier to access/edit.

    # LOOTBOX COMMAND / ERROR HANDLING
    @commands.cooldown(
        1, 360, commands.BucketType.user
    )  # 1 = number of times command can be used before cooldown is triggered, 10= number of seconds the cooldown is
    @commands.command(
        name="lootbox",
